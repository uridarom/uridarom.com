...
import 'piccolore';
import { c as createRenderInstruction, m as maybeRenderHead, a as renderTemplate, b as addAttribute, r as renderComponent } from './prerender_CIZgCjy4.mjs';
...
const $$SectionToggle = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="flex items-center justify-center gap-4 py-10 text-sm font-bold" style="font-family: var(--font-sans)"> <button id="toggle-photography" data-category="photography" class="text-white transition-opacity duration-200">
Photography
...
const $$MasonryGrid = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div id="masonry-grid" style="columns: 3; column-gap: var(--gutter);" class="w-full"> ${photos.map((photo) => renderTemplate`<a${addAttribute(`/photos/${photo.slug}`, "href")}${addAttribute(photo.data.category, "data-category")} style="display: block; break-inside: avoid; margin-bottom: var(--gutter);" class="group"> ${renderComponent($$result, "Image", $$Image, { "src": photo.data.image, "alt": photo.data.title, "widths": [400, 800], "sizes": "33vw", "class": "w-full block group-hover:opacity-85 transition-opacity duration-300", "loading": "lazy" })} </a>`)} </div>`;
